...
func (c *RegisterController) Post() {
	...
	logs.GetBeeLogger().Info("入参", username, nickname, password, repassword)
	...
	user2 := models.User{UserName: username, NickName: nickname, PassWord: password, CreateTime: time.Now().Unix()}
	err := models.InsertUser(user2)
	if err != nil {
		c.Data["json"] = map[string]interface{}{"code": 1, "message": "注册失败"}
	} else {
